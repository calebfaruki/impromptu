...
// RekorSearcher searches Rekor for entries by artifact digest.
type RekorSearcher struct {
	client rekorClient
}

func NewRekorSearcher(baseURL string) *RekorSearcher {
	return &RekorSearcher{client: newRekorClient(baseURL)}
}

func (s *RekorSearcher) Search(ctx context.Context, digest string) (*RekorEntry, error) {
	// POST to index/retrieve to find UUIDs by hash
	searchURL := s.client.baseURL + "/api/v1/index/retrieve"
	searchBody, _ := json.Marshal(map[string]string{"hash": digest})

	req, err := http.NewRequestWithContext(ctx, "POST", searchURL, strings.NewReader(string(searchBody)))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("rekor search returned HTTP %d", resp.StatusCode)
	}
	...
	entryURL := fmt.Sprintf("%s/api/v1/log/entries/%s", s.client.baseURL, uuids[0])
	return s.client.fetchAndParseEntry(ctx, entryURL)
}
